...
class Audio:
    ...
    async def audioset_cachemax(self, size: int):
        ...
        if size < self._cache_min():
            await self.bot.say(":warning: Sorry, but because of the number of servers"
    ...
    async def audioset_player(self):
        ...
        if self.settings["AVCONV"]:
            await self.bot.say(":ok: Player toggled. You're now using avconv.")
        else:
            await self.bot.say(":ok: Player toggled. You're now using ffmpeg.")
        self.save_settings()
    ...
    async def audioset_volume(self, ctx, percent: int=None):
        ...
        if percent is None:
            ...
            msg = ":headphones: ระดับเสียงคือ %d%%" % vol
        elif percent >= 0 and percent <= 200:
            self.set_server_setting(server, "VOLUME", percent)
            msg = ":headphones:ระดับเสียงเปลี่ยนเป็น %d%%." % percent
            if percent > 100:
    ...
    async def audioset_vote(self, ctx, percent: int):
        ...
        if percent < 0:
            await self.bot.say(":warning: ไม่สามารถต่ำว่า ศูนย์ ได้")
            return
        elif percent > 100:
            percent = 100
    ...
    async def audiostat_servers(self):
        ...
        await self.bot.say(":headphones: ขณะนี้กำลังเล่นเพลงอยู่ {} เซิฟเวอร์.".format(
            count))
    ...
    async def disconnect_all(self):
        ...
        await self.bot.say(":ok: เรียบร้อยค่ะ.")
    ...
    async def play_local(self, ctx, name):
        ...
        if not self.voice_connected(server):
            try:
                ...
                await self.bot.say(":warning: นายท่นต้องเข้า voice channel ก่อน เลเวีย"
                                   "ถึงจะสามารถเล่นเพลงได้")
                return
            except UnauthorizedConnect:
                await self.bot.say(":negative_squared_cross_mark: เลเวียไม่ได้รับอนุญาตให้เข้าร่วม"
                                   " voice channel ค่ะ")
                return
            except UnauthorizedSpeak:
                await self.bot.say(":negative_squared_cross_mark: เลเวียไม่ได้รับอนุญาต"
                                   "ให้พูดใน voice channel ค่ะ")
                return
            else:
                await self._join_voice_channel(voice_channel)
        ...
        if self.is_playing(server):
            await self.bot.say(":musical_note: เลเวียกำลังเล่นเพลงในเซิฟเวอร์นี้ค่ะ!")
        ...
        if self.currently_downloading(server):
            await self.bot.say(":inbox_tray: เลเวียกำลังดาวโหลดไฟล์ค่ะ")
            return

        lists = self._list_local_playlists()

        if not any(map(lambda l: os.path.split(l)[1] == name, lists)):
            await self.bot.say(":warning: ไม่พบ Local playlist ค่ะ")
            return
    ...
    async def list_local(self):
        ...
        if local_playlists:
            ...
            msg = msg.strip(", ")
            msg += "```"
            await self.bot.say(":headphones: local playlists ที่สามารถเล่นได้:\n{}".format(msg))
        else:
            await self.bot.say(":headphones: ไม่มี playlists ค่ะ")
    ...
    async def pause(self, ctx):
        ...
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say(":warning: ไม่มีเสียงเชื่อมต่อในเซิฟเวอร์นี้ค่ะ")
            return
        ...
        if not hasattr(voice_client, 'audio_player'):
            await self.bot.say(":warning: ไม่มีคิวการเล่นเพลงให้หยุดค่ะ")
        elif voice_client.audio_player.is_playing():
            voice_client.audio_player.pause()
            await self.bot.say(":pause_button: หยุดค่ะ")
        else:
            await self.bot.say(":warning: ไม่มีคิวการเล่นเพลงให้หยุดค่ะ")
    ...
    async def play(self, ctx, *, url_or_search_terms):
        ...
        if "." in url:
            if not self._valid_playable_url(url):
                await self.bot.say(":negative_squared_cross_mark: URL ผิดพลาดค่ะ")
                return
        else:
            url = url.replace("/", "&#47")
            url = "[SEARCH:]" + url
    ...
    async def prev(self, ctx):
        ...
        if self.is_playing(server):
            ...
            await self.bot.say(":track_previous: เล่นเพลงก่อนหน้าค่ะ")
        else:
            await self.bot.say(":warning: ไม่มีการเล่นเพลงในเซิฟเวอร์นี้ค่ะ")
    ...
    async def playlist_create(self, ctx, name):
        ...
        if not self._valid_playlist_name(name) or len(name) > 25:
            await self.bot.say(":warning: That playlist name is invalid. It must only"
        ...
        await self.bot.say(":ok: Empty playlist '{}' saved.".format(name))
    ...
    async def playlist_add(self, ctx, name, url):
        ...
        if self._valid_playable_url(url):
            try:
                await self.bot.say(":musical_note: กำลังนับรายการเพลง..."
                                   "กรุณารอสักครู่ค่ะ")
                songlist = await self._parse_playlist(url)
            except InvalidPlaylist:
                await self.bot.say(":warning: playlist URL ผิดพลาดค่ะ")
                return
    ...
    async def playlist_append(self, ctx, name, url):
        ...
        if name not in self._list_playlists(server):
            await self.bot.say(":warning: ไม่มี playlist ชื่อนี้ค่ะ.")
            return
        ...
        try:
            ...
            await self.bot.say(":warning: ไม่มีชื่อผู้แต่งของ playlist นี้ค่ะ")
        except InvalidURL:
            await self.bot.say(":warning: URL ผิดพลาดค่ะ")
        else:
            await self.bot.say(":ok: เรียบร้อยค่ะ")
    ...
    async def playlist_extend(self, ctx, playlist_url_or_name):
        ...
        # Need better wording ^
        await self.bot.say(":warning: ไม่สามารถดำเนินการได้ในขณะนี้")
    ...
    async def playlist_list(self, ctx):
        ...
        if files:
            ...
            msg = msg.strip(", ")
            msg += "```"
            await self.bot.say(":white_check_mark: playlist ที่สามารถเล่นได้:\n{}".format(msg))
        else:
            await self.bot.say(":warning: ไม่มี playlist ค่ะ")
    ...
    async def playlist_start(self, ctx, name):
        ...
        if self._playlist_exists(server, name):
            ...
            await self.bot.say(":ok: นำ playlist เข้าคิวแล้วค่ะ")
        else:
            await self.bot.say(":warning: ไม่มี playlist นี้ค่ะ")
    ...
    async def _queue_list(self, ctx):
        ...
        server = ctx.message.server
        if server.id not in self.queue:
            await self.bot.say(":warning: ไม่มีเพลงเล่นในเซิฟเวอร์นี้ค่ะ!")
            return
        elif len(self.queue[server.id]["QUEUE"]) == 0:
            await self.bot.say(":warning: ไม่มีคิวการเล่นเพลงในเซิฟเวอร์นี้ค่ะ")
            return
        ...
        await self.bot.say(":mag_right: กำลังรวบรวมข้อมูล...")
    ...
    async def repeat(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            if self.is_playing(server):
                ...
                await self.bot.say(":musical_note: กรุณาเล่นเพลงเพื่อดูการตั้งค่า")
    ...
    async def repeat_toggle(self, ctx):
        ...
        if repeat:
            await self.bot.say(":pause_button: ปิดการเล่นซ้ำค่ะ")
        else:
            await self.bot.say(":pause_button: เปิดการเล่นซ้ำค่ะ")
    ...
    async def resume(self, ctx):
        ...
        if not hasattr(voice_client, 'audio_player'):
            ...
            await self.bot.say(":play_pause: พักการเล่นค่ะ")
        else:
            await self.bot.say(":warning: ไม่มีคิวการเล่นเพลงให้หยุดค่ะ")
    ...
    async def _shuffle(self, ctx):
        ...
        server = ctx.message.server
        if server.id not in self.queue:
            await self.bot.say(":warning: ไม่มีคิวการเล่นเพลงให้สุ่มค่ะ")
            return

        self._shuffle_queue(server)
        self._shuffle_temp_queue(server)

        await self.bot.say(":twisted_rightwards_arrows: สุ่มคิวการเล่นค่ะ")
    ...
    async def skip(self, ctx):
        ...
        if self.is_playing(server):
            ...
            if msg.author.voice_channel == vchan:
                if self.can_instaskip(msg.author):
                    ...
                    if self._get_queue_repeat(server) is False:
                        self._set_queue_nowplaying(server, None)
                    await self.bot.say(":track_next: เล่นเพลงถัดไปค่ะ...")
                else:
            ...
            await self.bot.say(":warning: ไม่มีการเล่นเพลง ไม่สามารถข้ามได้ค่ะ")
    ...
    async def song(self, ctx):
        ...
        server = ctx.message.server
        if not self.is_playing(server):
            await self.bot.say(":warning: ไม่สามารถเล่นเพลงในเซิฟเวอร์นี้ได้ค่ะ")
            return
    ...
    async def yt_search(self, ctx, *, search_terms: str):
        ...
        await self.bot.say(":mag_right: กำลังค้นหาค่ะ...")
